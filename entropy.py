...
def load_bound_free(
  ...
  if bool_polymer:
    if atleast:
      bound_bottom = in_between_polymer_atleast(oxy_z, bound_low, ag_z.min(), atleast)
      bound_top = in_between_polymer_atleast(oxy_z, ag_z.max(), bound_high, atleast)
    else:
      bound_bottom = in_between_polymer_minmax(oxy_z, bound_low, ag_z.min())
      bound_top = in_between_polymer_minmax(oxy_z, ag_z.max(), bound_high)
  else:
    bound_bottom = in_between(oxy_z, bound_low, ag_z.min())
    bound_top = in_between(oxy_z, ag_z.max(), bound_high)
...
def in_between(value, low, high):
  return (value < high) * (value > low)

def in_between_polymer_minmax(input_list, low, high):
  array = np.array(input_list)
  return (array.max(axis=1) < high) * (array.min(axis=1) > low)

def in_between_polymer_atleast(array, low, high, atleast):
  return np.array([(sum(in_between(frame, low.min(), high.max())) >= atleast) for frame in array])
...
def solvent_traj(
  ...
  print('Bound dist = {0}'.format(bound_dist))
  print('Count as bound if at least {0} monomers are bound\n'.format(atleast))
  print('Binding potential energy = {0:.2f} +/- {1:.2f} kcal/mol'.format(bind_pot_kcal, bind_pot_sem_kcal))
  print('Binding free energy = {0:.2f} +/- {1:.2f} kcal/mol'.format(binding_free_kcal, binding_free_sem_kcal))
  print('Entropic contribution = {0:.2f} +/- {1:.2f} kcal/mol'.format(entro_kcal, entro_sem_kcal))
  print('Entropy at {0} K = {1:.4f} +/- {2:.4f} kcal/mol/T'.format(temp, entropy, entropy_sem))
...
def chemfiles_bound_free(
  ...
  oxy_type, ag_type,
  bound_dist=7,
  skipns=0, time_interval=2000, bool_polymer=False, atleast=None):
  ...
  if bool_polymer:
    oxy_z = [[pbc_wrap(x, box_z) for x in arr] for arr in oxy_z]
  else:
  ...
  if bool_polymer:
    if atleast:
      bound_bottom = in_between_polymer_atleast(oxy_z, bound_low, agbot_z, atleast)
      bound_top = in_between_polymer_atleast(oxy_z, agtop_z, bound_high, atleast)
    else:
      bound_bottom = in_between_polymer_minmax(oxy_z, bound_low, agbot_z)
      bound_top = in_between_polymer_minmax(oxy_z, agtop_z, bound_high)
  else:
    bound_bottom = in_between(oxy_z, bound_low, agbot_z)
    bound_top = in_between(oxy_z, agtop_z, bound_high)

  bound_combine = bound_top + bound_bottom
  
  if bool_polymer and atleast:
    bound_bottom_all = in_between_polymer_atleast(oxy_z, bound_low, agbot_z, atleast=1)
    bound_top_all = in_between_polymer_atleast(oxy_z, agtop_z, bound_high, atleast=1)
    bound_combine_all = bound_bottom_all + bound_top_all
    free_combine = [not x for x in bound_combine_all]
  else:
    free_combine = [not x for x in bound_combine]
